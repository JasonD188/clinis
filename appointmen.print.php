<?php
session_start();
include 'db.php';

$date_today = date("F d, Y h:i A");


if (!isset($_GET['id'])) {
    
    $allPatients = $conn->query("SELECT id, full_name, student_id FROM patients ORDER BY full_name ASC");
    ?>
    <!DOCTYPE html>
    <html>
    <head>
        <title>Select Patient</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    </head>
    <body class="p-4">
        <h3>Select a Patient to Print</h3>
        <div class="list-group mt-3">
            <?php while($p = $allPatients->fetch_assoc()): ?>
                <a href="?id=<?= $p['id'] ?>" class="list-group-item list-group-item-action">
                    <?= htmlspecialchars($p['full_name']) ?> (<?= htmlspecialchars($p['student_id']) ?>)
                </a>
            <?php endwhile; ?>
        </div>
    </body>
    </html>
    <?php
    exit;
}

$patient_id = intval($_GET['id']);


$patient = $conn->query("SELECT * FROM patients WHERE id = $patient_id");
if ($patient->num_rows == 0) {
    die("Patient not found.");
}
$patientData = $patient->fetch_assoc();


$appointments = $conn->query("
    SELECT * FROM appointments 
    WHERE patient_id = $patient_id
    ORDER BY appointment_date DESC, appointment_time DESC
");


$medical = $conn->query("
    SELECT * FROM medical_records
    WHERE patient_id = $patient_id
    ORDER BY record_date DESC
");
?>

<!DOCTYPE html>
<html>
<head>
<title>Patient Full Report</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">

<style>
body { font-size: 13px; }
.report-header { border-bottom: 2px solid #000; margin-bottom: 20px; padding-bottom: 10px; }
.logo { width: 80px; }
.section-title { margin-top: 40px; margin-bottom: 10px; font-weight: bold; font-size: 16px; border-bottom: 1px solid #000; padding-bottom: 5px; }
@media print { .no-print { display: none; } }
</style>
</head>
<body class="p-4">

<!-- HEADER -->
<div class="report-header d-flex align-items-center">
    <img src="./components/bestlink_college_of_the_philippines_logo.jpg" class="logo me-3">
    <div>
        <h4 class="mb-0">SCHOOL CLINIC MANAGEMENT SYSTEM</h4>
        <small><strong>Patient Full Report</strong></small><br>
        <small>Date Generated: <?= $date_today ?></small>
    </div>
</div>

<!-- PATIENT INFO -->
<div class="mb-3">
    <strong>Student ID:</strong> <?= htmlspecialchars($patientData['student_id']) ?><br>
    <strong>Full Name:</strong> <?= htmlspecialchars($patientData['full_name']) ?><br>
    <strong>Gender:</strong> <?= htmlspecialchars($patientData['gender']) ?><br>
    <strong>Blood Type:</strong> <?= htmlspecialchars($patientData['blood_type']) ?>
</div>

<div class="no-print mb-3">
    <button onclick="window.print()" class="btn btn-dark">
        Print / Save as PDF
    </button>
</div>

<div class="section-title">Appointments</div>
<table class="table table-bordered">
<thead class="table-dark">
<tr>
    <th>#</th>
    <th>Date</th>
    <th>Time</th>
    <th>Doctor</th>
    <th>Type</th>
    <th>Status</th>
</tr>
</thead>
<tbody>
<?php 
$i=1; 
if($appointments->num_rows > 0):
while($r=$appointments->fetch_assoc()):
?>
<tr>
    <td><?= $i++ ?></td>
    <td><?= date('M d, Y', strtotime($r['appointment_date'])) ?></td>
    <td><?= date('h:i A', strtotime($r['appointment_time'])) ?></td>
    <td><?= htmlspecialchars($r['doctor_name']) ?></td>
    <td><?= $r['type'] ?></td>
    <td><?= $r['status'] ?></td>
</tr>
<?php endwhile; else: ?>
<tr><td colspan="6" class="text-center">No appointments found.</td></tr>
<?php endif; ?>
</tbody>
</table>

<div class="section-title">Medical Records</div>
<table class="table table-bordered">
<thead class="table-dark">
<tr>
    <th>#</th>
    <th>Date</th>
    <th>Vitals</th>
    <th>Diagnosis</th>
    <th>Prescription</th>
</tr>
</thead>
<tbody>
<?php 
$i=1; 
if($medical->num_rows > 0):
while($r=$medical->fetch_assoc()):
?>
<tr>
    <td><?= $i++ ?></td>
    <td><?= date('M d, Y h:i A', strtotime($r['record_date'])) ?></td>
    <td>
        <?php if($r['bp']) echo "BP: ".$r['bp']."<br>"; ?>
        <?php if($r['weight']) echo "Weight: ".$r['weight']."kg<br>"; ?>
        <?php if($r['height']) echo "Height: ".$r['height']."cm<br>"; ?>
        <?php if($r['temperature']) echo "Temp: ".$r['temperature']."°C"; ?>
    </td>
    <td><?= htmlspecialchars($r['diagnosis']) ?></td>
    <td><?= htmlspecialchars($r['prescription']) ?></td>
</tr>
<?php endwhile; else: ?>
<tr><td colspan="5" class="text-center">No medical records found.</td></tr>
<?php endif; ?>
</tbody>
</table>

<br><br>

<div class="row mt-5">
    <div class="col-6 text-center">
        ___________________________<br>
        <strong>Clinic Nurse</strong>
    </div>
    <div class="col-6 text-center">
        ___________________________<br>
        <strong>Physician</strong>
    </div>
</div>

<script>
window.onload = function() {
    window.print();
}
</script>

</body>
</html>